...
export function ClinixAIChatScreen({ role, title, subtitle }) {
    ...
    const onSubmit = handleSubmit(async (values) => {
        ...
        try {
            const response = await chatMutation.mutateAsync({
                ...
                patientContext: role === 'doctor' && usePatientContext && values.patientContext.trim()
    ...
    if (isHydrating) {
        return (<Screen title={title} subtitle="Loading local AI chat..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    return (<Screen title={title} subtitle={subtitle}>
      <Card>
      ...
      {role === 'doctor' ? (<Card>
          ...
          {usePatientContext ? (<Controller control={control} name="patientContext" render={({ field: { onChange, onBlur, value }, fieldState: { error } }) => (<View style={styles.inputField}>
                  ...
                  <TextInput accessibilityLabel="Patient context" multiline onBlur={onBlur} onChangeText={onChange} placeholder="e.g. 68M, uncontrolled diabetes, CKD stage 3, penicillin allergy..." placeholderTextColor={colors.textMuted} style={[styles.textArea, styles.contextArea, error && styles.errorBorder]} textAlignVertical="top" value={value}/>
      ...
      <Card>
        ...
        <View style={styles.quickPromptRow}>
          {quickPrompts.map((prompt) => (<Pressable key={prompt} onPress={() => setValue('prompt', prompt, { shouldValidate: true })} style={styles.quickPrompt}>
              <Text style={styles.quickPromptText}>{prompt}</Text>
            </Pressable>))}
        </View>

        {messages.length === 0 ? (<EmptyState title="No AI messages yet" subtitle="Ask CliniX AI a question to start this conversation."/>) : (<View style={styles.messagesList}>
            {messages.map((message) => (<View key={message.id} style={[styles.bubble, message.role === 'user' ? styles.userBubble : styles.aiBubble]}>
            ...
            {chatMutation.isPending ? (<View style={[styles.bubble, styles.aiBubble]}>
      ...
      <Card>
        <Controller control={control} name="prompt" render={({ field: { onChange, onBlur, value }, fieldState: { error } }) => (<View style={styles.inputField}>
              <Text style={styles.inputLabel}>
                {role === 'doctor' ? 'Ask for note/prescription drafting help' : 'Ask about results or medication use'}
              </Text>
              <TextInput accessibilityLabel="AI chat prompt" multiline onBlur={onBlur} onChangeText={onChange} placeholder={role === 'doctor'
                ? 'Example: Draft a concise plan for hypertension follow-up.'
                : 'Example: Explain this result and what I should do next.'} placeholderTextColor={colors.textMuted} style={[styles.textArea, error && styles.errorBorder]} textAlignVertical="top" value={value}/>
              {error?.message ? <Text style={styles.errorText}>{error.message}</Text> : null}
            </View>)}/>

        <PrimaryButton label={chatMutation.isPending ? 'CliniX AI is typing...' : 'Send to CliniX AI'} loading={chatMutation.isPending} disabled={!canSend} onPress={onSubmit}/>
      </Card>
    </Screen>);
}
