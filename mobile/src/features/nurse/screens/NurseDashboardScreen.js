...
export function NurseDashboardScreen() {
    ...
    if (isLoading) {
        return (<Screen title="Nurse Dashboard" subtitle="Loading ward assignments..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    const highRiskCount = data?.filter((patient) => patient.riskLevel === 'high').length ?? 0;
    return (<Screen title={`Welcome, ${session?.user.profile.fullName ?? 'Nurse'}`} subtitle="Track assigned patients, priorities, and alerts from one place." refreshing={isRefetching} onRefresh={() => {
            void refetch();
        }}>
      <Card>
