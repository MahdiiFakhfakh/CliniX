...
export function PatientRecordsScreen() {
    ...
    const isRefreshing = profileQuery.isRefetching ||
    ...
    const isLoading = profileQuery.isLoading || summaryQuery.isLoading || prescriptionsQuery.isLoading || resultsQuery.isLoading;
    ...
    if (isLoading) {
        return (<Screen title="Medical Records" subtitle="Loading profile and results..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    return (<Screen title="Medical Records" subtitle="Profile, prescriptions, and lab/imaging summaries." refreshing={isRefreshing} onRefresh={handleRefresh}>
      <Card>
      ...
      <Card>
        ...
        {!prescriptionsQuery.data || prescriptionsQuery.data.length === 0 ? (<EmptyState title="No prescriptions" subtitle="New prescriptions will appear here."/>) : (<View style={styles.list}>
            {prescriptionsQuery.data.map((prescription) => (<View key={prescription.id} style={styles.itemRow}>
      ...
      <Card>
        ...
        {!resultsQuery.data || resultsQuery.data.length === 0 ? (<EmptyState title="No results" subtitle="Completed results will appear here."/>) : (<View style={styles.list}>
            {resultsQuery.data.map((result) => (<View key={result.id} style={styles.resultRow}>
                <View>
                ...
                <PrimaryButton label={explainMutation.isPending ? 'Explaining...' : 'Explain with AI'} loading={explainMutation.isPending} onPress={() => handleExplain(result)}/>
              </View>))}
          </View>)}
      </Card>
