...
export function PatientResultsListScreen() {
    ...
    if (resultsQuery.isLoading) {
        return (<Screen title="Results" subtitle="Loading lab and imaging results..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    if (!resultsQuery.data || resultsQuery.data.length === 0) {
        return (<Screen title="Results" subtitle="No available results." refreshing={resultsQuery.isRefetching} onRefresh={handleRefresh}>
        <EmptyState title="No results" subtitle="Your lab and imaging results will appear here."/>
      </Screen>);
    }
    return (<Screen title="Results" subtitle="Open any result for detailed interpretation." refreshing={resultsQuery.isRefetching} onRefresh={handleRefresh}>
      <PrimaryButton label="Open CliniX AI" onPress={() => router.push('/(app)/(patient)/clinix-ai')}/>

      <View style={styles.list}>
        {resultsQuery.data.map((result) => (<Pressable key={result.id} onPress={() => router.push({
