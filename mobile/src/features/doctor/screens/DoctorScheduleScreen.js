...
export function DoctorScheduleScreen() {
    ...
    if (appointmentsQuery.isLoading) {
        return (<Screen title="Schedule Agenda" subtitle="Loading day/week agenda..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    return (<Screen title="Schedule Agenda" subtitle="Agenda view for day and week planning." refreshing={appointmentsQuery.isRefetching} onRefresh={handleRefresh}>
      ...
      {agendaAppointments.length === 0 ? (<EmptyState title="No appointments" subtitle={`No ${viewMode} appointments found.`}/>) : (<View style={styles.list}>
          {agendaAppointments.map((appointment) => (<View key={appointment.id} style={styles.agendaItem}>
              ...
              {appointment.patientId ? (<Pressable accessibilityRole="button" onPress={() => router.push({
