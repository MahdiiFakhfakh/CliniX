...
function toBackendAppointment(appointment) {
    return {
        _id: appointment.id,
...
function toBackendPatientItem(patientId) {
    ...
    return {
        ...
        status: mapRiskToStatus(getMockDoctorPatients().find((item) => item.id === patientId)?.riskLevel ?? 'low'),
        updatedAt: new Date().toISOString(),
    };
}
function buildThreadTitle(role) {
    if (role === 'doctor') {
...
function resolveRoleFromThreadId(threadId, fallback) {
...
function getThreadMessagesById(threadId, fallbackRole) {
...
async function handlePatientAndDoctorRoutes(context, pathParams) {
    ...
    if (appointmentMatch && method === 'PATCH') {
        ...
        const appointment = incomingStatus === 'cancelled'
...
async function handleMessagingRoutes(context) {
...
async function handleNotificationAndAIRoutes(context) {
...
export function asApiClientError(error) {
    ...
    const normalized = new Error(mockError.message || 'Unexpected mock server error');
    normalized.statusCode = mockError.statusCode;
    return normalized;
}
export function mapMockStatusToRisk(status) {
