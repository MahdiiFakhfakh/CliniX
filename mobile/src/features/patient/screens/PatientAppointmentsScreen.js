...
export function PatientAppointmentsScreen() {
    ...
    if (appointmentsQuery.isLoading) {
        return (<Screen title="Appointments" subtitle="Loading your appointment list..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    if (!appointmentsQuery.data || appointmentsQuery.data.length === 0) {
        return (<Screen title="Appointments" subtitle="No scheduled visits yet." refreshing={appointmentsQuery.isRefetching} onRefresh={handleRefresh}>
    ...
    return (<Screen title="Appointments" subtitle="Tap any appointment to open details." refreshing={appointmentsQuery.isRefetching} onRefresh={handleRefresh}>
      <PrimaryButton label="Book Appointment" onPress={() => router.push('/(app)/(patient)/book-appointment')}/>

      <View style={styles.list}>
        {appointmentsQuery.data.map((appointment) => (<Pressable key={appointment.id} accessibilityRole="button" onPress={() => router.push({
