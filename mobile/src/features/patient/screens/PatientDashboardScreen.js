...
export function PatientDashboardScreen() {
    ...
    const isRefreshing = appointmentsQuery.isRefetching || profileQuery.isRefetching || summaryQuery.isRefetching || resultsQuery.isRefetching;
    const handleRefresh = () => {
        void Promise.all([
    ...
    if (appointmentsQuery.isLoading || profileQuery.isLoading || summaryQuery.isLoading || resultsQuery.isLoading) {
        return (<Screen title="Patient Home" subtitle="Loading your care overview..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    const nextAppointment = appointmentsQuery.data?.[0];
    const recentResults = (resultsQuery.data ?? []).slice(0, 2);
    return (<Screen title={`Welcome, ${profileQuery.data?.fullName.split(' ')[0] ?? 'Patient'}`} subtitle="Next appointment, recent results, and quick actions." refreshing={isRefreshing} onRefresh={handleRefresh}>
      <Card>
        ...
        {nextAppointment ? (<>
            ...
            <PrimaryButton label="View Appointment Details" onPress={() => router.push({
      ...
      <Card>
        ...
        {recentResults.length === 0 ? (<Text style={styles.metaText}>No recent results.</Text>) : (<View style={styles.resultList}>
            {recentResults.map((result) => (<View key={result.id} style={styles.resultRow}>
