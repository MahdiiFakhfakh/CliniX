...
export function NotificationsScreen() {
    ...
    if (isLoading) {
        return (<Screen title="Notifications List" subtitle="Loading feed..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    return (<Screen title="Notifications List" subtitle="Uses expo-notifications with local fallback." refreshing={isRefetching} onRefresh={() => {
            void refetch();
        }}>
      <PrimaryButton label="Schedule Test Reminder" onPress={handleTestReminder}/>

      {!data || data.length === 0 ? (<EmptyState title="No notifications" subtitle="You are all caught up."/>) : (<View style={styles.list}>
