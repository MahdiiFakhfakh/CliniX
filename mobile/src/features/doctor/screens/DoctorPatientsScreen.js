...
export function DoctorPatientsScreen() {
    ...
    if (patientsQuery.isLoading) {
        return (<Screen title="Patient List" subtitle="Loading assigned patients..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    if (!patientsQuery.data || patientsQuery.data.length === 0) {
        return (<Screen title="Patient List" subtitle="No patients assigned." refreshing={patientsQuery.isRefetching} onRefresh={handleRefresh}>
        <EmptyState title="No patients" subtitle="Patient assignments will appear here."/>
      </Screen>);
    }
    return (<Screen title="Patient List" subtitle="Open a patient to review history and create orders." refreshing={patientsQuery.isRefetching} onRefresh={handleRefresh}>
      <View style={styles.list}>
        {patientsQuery.data.map((patient) => (<Pressable key={patient.id} accessibilityRole="button" onPress={() => router.push({
