...
export function PatientPrescriptionDetailScreen() {
    ...
    if (prescriptionsQuery.isLoading) {
        return (<Screen title="Prescription Details" subtitle="Loading prescription..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    const prescription = prescriptionsQuery.data?.find((item) => item.id === prescriptionId);
    if (!prescription) {
        return (<Screen title="Prescription Details" subtitle="Prescription not found.">
        <EmptyState title="Not found" subtitle="This prescription is unavailable."/>
      </Screen>);
    }
    return (<Screen title="Prescription Details" subtitle="Medication instructions and schedule.">
      <Card>
