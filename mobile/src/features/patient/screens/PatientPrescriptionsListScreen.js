...
export function PatientPrescriptionsListScreen() {
    ...
    if (prescriptionsQuery.isLoading) {
        return (<Screen title="Prescriptions" subtitle="Loading prescriptions..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    if (!prescriptionsQuery.data || prescriptionsQuery.data.length === 0) {
        return (<Screen title="Prescriptions" subtitle="No prescriptions found." refreshing={prescriptionsQuery.isRefetching} onRefresh={handleRefresh}>
        <EmptyState title="No prescriptions" subtitle="Your medication list will appear here."/>
      </Screen>);
    }
    return (<Screen title="Prescriptions" subtitle="Open any prescription to view details." refreshing={prescriptionsQuery.isRefetching} onRefresh={handleRefresh}>
      <View style={styles.list}>
        {prescriptionsQuery.data.map((prescription) => (<Pressable key={prescription.id} onPress={() => router.push({
