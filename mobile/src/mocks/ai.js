export function mockExplainResult(payload) {
    return {
        title: 'AI Result Explanation',
        content: `For result ${payload.resultId}: this appears mildly abnormal but not immediately dangerous. ` +
...
export function mockDraftClinicalText(payload) {
    if (payload.kind === 'consultation-note') {
        return {
            title: 'Draft Consultation Note',
            content: 'Subjective: patient reports gradual symptom improvement. Objective: stable vitals and no acute distress. ' +
    ...
    return {
        ...
        content: 'Medication: choose guideline-aligned first-line therapy. Dose: start low and titrate. Frequency: once daily. ' +
...
export function mockAIChat(payload) {
    const lastMessage = payload.messages[payload.messages.length - 1];
    const prompt = lastMessage?.content ?? '';
    const contextSuffix = payload.patientContext ? ` Patient context: ${payload.patientContext}.` : '';
    if (payload.role === 'doctor') {
        return {
            content: 'Draft suggestion: summarize key findings, add focused assessment, and include an actionable care plan. ' +
    ...
    return {
        content: 'I can help explain results in plain language and provide medication-use guidance. ' +
