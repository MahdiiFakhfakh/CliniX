...
export function PatientResultDetailScreen() {
    ...
    if (resultsQuery.isLoading) {
        return (<Screen title="Result Details" subtitle="Loading result..." scroll={false}>
        <LoadingView />
      </Screen>);
    }
    const result = resultsQuery.data?.find((item) => item.id === resultId);
    if (!result) {
        return (<Screen title="Result Details" subtitle="Result not found.">
        <EmptyState title="Not found" subtitle="This result is unavailable."/>
      </Screen>);
    }
    return (<Screen title="Result Details" subtitle="Clinical summary and next actions.">
      <Card>
      ...
      <PrimaryButton label="Explain with CliniX AI" onPress={() => router.push({
